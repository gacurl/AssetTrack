...
| Asset import | `/admin/assets/import` | `admin_asset_import.html`, `assettrack/import_analysis.py`, `assettrack/import_reconciliation.py` | Normal CSV/XLSX asset import workflow for Laptop, Switch, and Router records. | Admins | Core initial load | Core initial load | Creates asset/event records | No direct receipt unless Issue mode | Import analysis/reconciliation, ingest committer | Harder to find import | Cannot onboard assets | Keep visible in Admin | None |
| Network-device CSV import | CLI/script, docs fixtures | `assettrack/network_asset_import.py`, `scripts/import_network_assets_csv.py`, network fixture docs | Deprecated internal switch/router CSV utility; rejects CMDB fields and is not the normal operator workflow. | Maintainers only | Not relevant | Legacy support reference | Creates physical custody assets via ingest path | No direct receipt | Existing slots, ingest committer | Hidden CLI may be missed by maintainers | Legacy recovery becomes manual | Keep internal/deprecated | Issue 30-22: retain with deprecation boundary |
| Network-device staging docs | legacy CSV/MD | `docs/legacy/network_switch_router_staging_template_v1.*` | Historical Switch/Router-only staging contract. | Maintainers only | Not relevant | Legacy support reference | Preserves custody-only boundary | None | Deprecated internal import script | Operators may mistake it for current import contract | Risk of ad hoc CMDB data | Quarantined in legacy docs | Issue 30-26: keep out of canonical operator docs |
...
| Public demo page | `/demo`, `/demo/send-sample-receipt` | `demo.html`, demo helpers | Separate public demo/support surface with static sample data and optional demo-only receipt send. | Public/support | Marketing/support only | Marketing/support only | No custody writes; no operational data | Demo receipt only; no receipt record created | SMTP, token env only when sample sending is enabled | No core impact | Product demo removed | Keep separate from operational deployment | Issue 29-10: public demo boundary is Separate |
...
3. If onboarding many switches/routers, use Admin Tools -> Import Assets with a CSV or XLSX file for Switch and Router records; do not add CMDB fields.
...
| Network staging template | docs fixture | Legacy support reference | Input control | Import validation | No | No | Yes | Not a report; staging artifact | Keep internal/deprecated | Issue 30-22: retain with deprecation boundary |
...
| Issue 30-22: Network import deprecation boundary | Keep the legacy CLI network import internal/deprecated while normal onboarding uses Import Assets. | Network devices are core, but CMDB scope must stay out. |
...
- Whether any future network-device import change is needed beyond the current internal/deprecated CLI boundary and normal Import Assets workflow.
