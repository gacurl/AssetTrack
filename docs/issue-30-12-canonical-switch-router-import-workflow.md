...
| `/admin/network-assets/import/template.csv` and `docs/fixtures/imports/network/network_switch_router_staging_template_v1.csv` | Legacy network CSV reference artifact | The file documents the older Switch/Router-only CSV contract used by the network CLI. It is not the canonical Import Assets CSV/XLSX workflow. |
| `docs/fixtures/imports/network/network_switch_router_staging_template_v1.md` | Legacy network CSV reference documentation | Documents the older CLI-fed staging contract and CMDB exclusions. It remains useful for classifying the older utility, but it is not the canonical operator workflow. |
